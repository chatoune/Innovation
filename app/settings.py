...
class AppSettings:
    ...
    KEY_CONFIG_DB_PATH = "config/file_path"
    ...
    def get_config_db_path(self) -> Optional[Path]:
        raw_value = self._settings.value(self.KEY_CONFIG_DB_PATH, None)
        if raw_value is None:
            return None
        return Path(str(raw_value)).expanduser()

    def set_config_db_path(self, db_path: str | Path) -> None:
        self._settings.setValue(self.KEY_CONFIG_DB_PATH, str(Path(db_path).expanduser()))
        self._settings.sync()

    def clear_config_db_path(self) -> None:
        self._settings.remove(self.KEY_CONFIG_DB_PATH)
        self._settings.sync()